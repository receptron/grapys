  ...
  return {
    ...
    nodeOutputClass: (expectNearNode, nodeData, isConnectable = true) => {
      ...
      return (
        (expectNearNode
          ? isConnectable
            ? (portColors.outputHighlight ?? outputHighlight)
            : (portColors.notConnectable ?? notConnectable)
          : (portColors.output ?? output)) ?? ""
      );
    },
    nodeInputClass: (expectNearNode, nodeData, isConnectable = true) => {
      ...
      return (
        (expectNearNode
          ? isConnectable
            ? (portColors.inputHighlight ?? inputHighlight)
            : (portColors.notConnectable ?? notConnectable)
          : (portColors.input ?? input)) ?? ""
      );
    },
  };
};
